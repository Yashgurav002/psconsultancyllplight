...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            >
              ...
              <div>
                ...
                <div className="font-body text-[10px] text-pitch-500 tracking-widest uppercase leading-none">
                  Sports Consultancy
                </div>
              </div>
            </a>
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-5 py-2.5 bg-pitch-600 hover:bg-pitch-500 text-white text-sm font-heading font-semibold rounded-full transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-pitch-600/30"
              >
                GET STARTED
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 z-40 lg:hidden mobile-menu ${menuOpen ? 'open' : 'closed'}`}
      >
        ...
        <div className="relative h-full flex flex-col items-center justify-center gap-2 p-8">
          <div className="absolute top-6 left-6 font-heading font-bold text-xl text-white">
            PIONEER<span className="text-pitch-500 text-sm ml-1">SC</span>
          </div>
          {navLinks.map((link, i) => (
            <a
              key={link.href}
              href={link.href}
              onClick={(e) => { e.preventDefault(); handleNavClick(link.href) }}
              className="w-full max-w-xs text-center py-3 text-2xl font-heading font-bold text-white hover:text-pitch-400 transition-colors border-b border-pitch-950/40 last:border-0"
          ...
          <a
            href="#contact"
            onClick={(e) => { e.preventDefault(); handleNavClick('#contact') }}
            className="mt-6 px-8 py-3.5 bg-pitch-600 hover:bg-pitch-500 text-white text-sm font-heading font-semibold rounded-full transition-colors"
          >
            GET STARTED
          </a>
        </div>
