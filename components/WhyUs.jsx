...
const differentiators = [
  {
    number: '01', title: 'International Exposure',
    ...
    accent: '#3D6B4F', bg: '#EDF4EF', border: '#D4E8DA',
  },
  {
    number: '02', title: 'Expert Methodology',
    ...
    accent: '#C17D2E', bg: '#FBF3E8', border: '#F5E0C0',
  },
  {
    number: '03', title: 'Global Network',
    ...
    icon: (<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>),
    accent: '#5A8A6A', bg: '#F4F8F5', border: '#D4E8DA',
  },
  {
    number: '04', title: 'Personalized Plans',
    ...
    accent: '#A86520', bg: '#FDF7EE', border: '#E8C08A',
  },
  {
    number: '05', title: 'Proven Track Record',
...
export default function WhyUs() {
  ...
  return (
    <section id="why-us" className="relative py-24 md:py-32 bg-neutral-50 overflow-hidden">
      <div className="absolute inset-0"
        style={{ background: 'radial-gradient(ellipse 40% 60% at 100% 50%, rgba(193,125,46,0.04) 0%, transparent 55%)' }}/>

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={titleRef} className={`reveal mb-16 ${titleVisible ? 'visible' : ''}`}>
          <div className="grid lg:grid-cols-2 gap-8 items-end">
            <div>
              <div className="flex items-center gap-3 mb-5">
                <div className="h-px w-8 bg-sage-300"/>
                <span className="text-sage-500 text-xs font-heading font-semibold tracking-widest uppercase">The Pioneer Difference</span>
              </div>
              <h2 className="font-heading font-black text-4xl md:text-5xl text-neutral-900 leading-tight">
          ...
          {differentiators.map((d, i) => {
            ...
            return (
              <div
                key={i}
                ref={ref}
                className={`reveal card-hover rounded-2xl p-6 md:p-8 border ${visible ? 'visible' : ''}`}
                style={{ background: d.bg, borderColor: d.border, transitionDelay: `${i * 0.08}s`, borderLeft: `4px solid ${d.accent}` }}
              >
                <div className="flex items-start gap-5 md:gap-8">
                  <div className="font-heading font-black text-3xl md:text-4xl flex-shrink-0 w-12 text-right leading-none"
                    style={{ color: `${d.accent}30` }}>
                    {d.number}
                  </div>
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 bg-white shadow-sm border"
                    style={{ color: d.accent, borderColor: d.border }}>
                    {d.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-heading font-bold text-lg md:text-xl text-neutral-900 mb-2">{d.title}</h3>
                    <p className="text-neutral-600 font-body text-sm leading-relaxed">{d.desc}</p>
                  </div>
                </div>
              </div>
            )
        ...
        <div className="text-center mt-14">
          <button
            ...
            className="inline-flex items-center gap-2.5 px-8 py-4 bg-sage-600 hover:bg-sage-700 text-white font-heading font-bold text-sm tracking-wide rounded-full transition-all duration-300 hover:scale-105 shadow-sage"
          >
